...
import { ShoppingCart, Plus, Search, Eye, CheckCircle, XCircle, Clock, Truck, FileCheck } from 'lucide-react';
...
const STATUS_MAP = {
    ...
    processing:{ label: 'En proceso', color: 'bg-blue-50 text-blue-700',        icon: Truck },
    partial:   { label: 'Parcial',    color: 'bg-orange-50 text-orange-600',    icon: Truck },
    delivered: { label: 'Entregada',  color: 'bg-emerald-50 text-emerald-700',  icon: CheckCircle },
    invoiced:  { label: 'Facturada',  color: 'bg-purple-50 text-purple-700',    icon: FileCheck },
...
export default function SalesOrdersIndex({ orders, stats, clients, filters }) {
    const { flash } = usePage().props;
    const [search, setSearch] = useState(filters.search ?? '');

    function applyFilter(extra) {
        router.get('/ventas', { ...filters, search, ...extra }, { preserveState: true, replace: true });
    }
        ...
        <AppLayout title="Ventas">
            <div className="space-y-5">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-xl font-bold text-[#2B2B2B]">Órdenes de venta</h1>
                        <p className="text-sm text-[#2B2B2B]/50 mt-0.5">Seguimiento de ventas y entregas</p>
                    </div>
                    <div className="flex gap-2">
                        <Link href="/presupuestos/crear"><Button variant="outline"><Plus size={15} /> Presupuesto</Button></Link>
                        <Link href="/ventas/crear"><Button><Plus size={15} /> Nueva venta</Button></Link>
                    </div>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    <StatCard label="Total" value={stats.total} status="total" onClick={() => applyFilter({ status: undefined })} active={!filters.status} />
                    {['pending', 'partial', 'delivered', 'invoiced'].map(s => (
                ...
                <div className="bg-white border border-[#D3D3D3] rounded-xl p-4">
                    <form onSubmit={e => { e.preventDefault(); applyFilter({}); }} className="flex flex-wrap gap-2">
                        ...
                        <AppSelect value={filters.client_id ? String(filters.client_id) : ''} onValueChange={val => applyFilter({ client_id: val || undefined })}
                            options={clients.map(c => ({ value: c.id, label: c.name }))} placeholder="Todos los clientes" />
                        <AppSelect value={filters.status ?? ''} onValueChange={val => applyFilter({ status: val || undefined })}
                            options={Object.entries(STATUS_MAP).map(([v, { label }]) => ({ value: v, label }))} placeholder="Todos los estados" />
                        <AppSelect value={filters.payment_type ?? ''} onValueChange={val => applyFilter({ payment_type: val || undefined })}
                            options={Object.entries(PAYMENT_LABELS).map(([v, label]) => ({ value: v, label }))} placeholder="Forma de pago" />
                        {(filters.search || filters.status || filters.client_id || filters.payment_type) && (
                            <Button type="button" variant="ghost" onClick={() => router.get('/ventas', {}, { replace: true })} className="text-xs text-[#2B2B2B]/50">Limpiar</Button>
                        )}
                    </form>
                </div>
                ...
                <div className="bg-white border border-[#D3D3D3] rounded-xl overflow-hidden">
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-[#D3D3D3]/60 bg-[#F4F4F4]/60">
                                ...
                                <th className="px-4 py-3 text-left text-xs font-semibold text-[#2B2B2B]/50 uppercase tracking-wide">Pago</th>
                        ...
                        <tbody className="divide-y divide-[#D3D3D3]/40">
                            {orders.data.length === 0 && (
                                <tr><td colSpan={7} className="px-4 py-12 text-center text-[#2B2B2B]/35 text-sm">No se encontraron órdenes de venta.</td></tr>
                            )}
                            {orders.data.map(o => {
                                ...
                                return (
                                    <tr key={o.id} className="hover:bg-[#F4F4F4]/60 transition-colors group">
                                        <td className="px-4 py-3 font-mono text-xs text-[#2D4C73] font-semibold">
                                            <Link href={`/ventas/${o.id}`} className="hover:underline">{o.number}</Link>
                                        </td>
                                        <td className="px-4 py-3 text-[#2B2B2B]">{o.client}</td>
                                        <td className="px-4 py-3 text-xs text-[#2B2B2B]/60">{o.date}</td>
                                        <td className="px-4 py-3 text-xs text-[#2B2B2B]/60">{PAYMENT_LABELS[o.payment_type]}</td>
                                        <td className="px-4 py-3 text-center">
                                            <span className={cn('text-[11px] font-medium px-2 py-0.5 rounded-full', st.color)}>{st.label}</span>
                                        </td>
                    ...
                    {orders.last_page > 1 && (
                        <div className="px-4 py-3 border-t border-[#D3D3D3]/60 flex items-center justify-between">
                            <span className="text-xs text-[#2B2B2B]/40">{orders.from}–{orders.to} de {orders.total}</span>
